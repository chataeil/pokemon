...
public class BoardAuthService {
    ...
    public void check(String mode, String bid, Long seq){
        ...
        if (List.of("write", "list").contains(mode)) {
        ...
        } else if (List.of("view").contains(mode)) {
          authority = board.getViewAuthority();
             ...
             *
             * 2. 비회우너 게시글인 경우 / 비회원 비밀번호 확인이 완료된 경우 삭제 가능
             */
            ...
            if (poster == null) { // 비회원 게시글
                /**
                 * 비회원 게시글이 인증된 경우 - 세션 키 - "board_게시글번호가 존재"
                 * 인증이 되지 않은 경우 GuestPasswordCheckExceptio을 발생 시킨다 -> 비번 확인 절차
                 */
                if (session.getAttribute("board_" + seq) == null){
                    session.setAttribute("seq", seq);
                    throw new GuestPasswordCheckException();
                }
            }else if (!memberUtil.isLogin() || poster.getEmail().equals(member.getEmail())) { // 회원 게시글 - 직접 작성한 회원만 수정 가능 통제 - 미로그인 상태 또는 로그인 상태지만 작성자의 이메일과 일치하지 않는 경우
                isVerified = false;
            }
        }
