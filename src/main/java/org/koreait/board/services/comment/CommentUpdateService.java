...
public class CommentUpdateService {
    ...
    public CommentData save(RequestComment form){
        ...
        if (mode.equals("edit") && seq != null && seq > 0L) { // 댓글 수정
            item = repository.findById(seq).orElseThrow(CommentNotFoundException::new);
        ...
        item.setCommenter(form.getContent());
    ...
    public void updateCommentCount(Long seq){
        ...
        long total = repository.count(commentData.data.seq.eq(seq)); // 게시글별 댓글 갯수
